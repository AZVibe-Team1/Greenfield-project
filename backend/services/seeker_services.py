...
class SeekerService:
    ...
    async def create_new_seeker(
        ...
        - seeker_id: MongoDB ObjectId (auto-generated by model)
        - seeker_identification: UUID for ChromaDB operations
        ...
        """
        try:
            # Generate unique Seeker Identification (UUID for ChromaDB)
            seeker_identification = uuid4()
            ...
            try:
                ...
                logger.error(f"Validation error creating Information for {email}: {validation_error}")
                raise

            # Prepare seeker data
            # Note: seeker_id (MongoDB ObjectId) is auto-generated by the Seeker model
            created_at = datetime.now(ZoneInfo("America/Denver"))
            seeker_data = {
                "seeker_identification": seeker_identification,
                "temperature": 0.75,
            ...
            # Write to ChromaDB: Resume
            # Always write to Resume collection (required for recommendations)
            # If no resume provided, create a profile summary from available data
            if resume:
                resume_text = resume
            else:
                # Create a profile summary from skills and education
                profile_parts = []
                if first_name and last_name:
                    profile_parts.append(f"{first_name} {last_name}")
                if education_level and edu_focus:
                    profile_parts.append(f"Education: {education_level} in {edu_focus}")
                elif education_level:
                    profile_parts.append(f"Education: {education_level}")
                elif edu_focus:
                    profile_parts.append(f"Field of Study: {edu_focus}")
                if key_skills:
                    profile_parts.append(f"Skills: {', '.join(key_skills)}")
                resume_text = ". ".join(profile_parts) if profile_parts else "Job Seeker Profile"
            
            metadata_resume = {"Post_Date": str(created_at)}
            write_collection(
                collection=Seeker_Resume_collection,
                collection_name="Seeker_Resume_Collection",
                id_value=str(seeker_identification),
                text_value=resume_text,
                metadata=metadata_resume
            )

            # Write to ChromaDB: Skills
            if key_skills or education_level or edu_focus:
                # Concatenate skills data
                skills_parts = []
                if key_skills:
                    skills_parts.append(", ".join(key_skills))
                if education_level:
            ...
            logger.error(f"Error creating seeker {first_name} {last_name} ({email}): {e!s}")
            import traceback
    ...
    async def upload_resume(
        ...
        try:
            ...
            # Update seeker record using update_seeker_profile to ensure
            # both MongoDB and ChromaDB are updated
            update_data = {
                "resume": resume_text
            }

            return await SeekerService.update_seeker_profile(seeker_id, update_data)
    ...
    async def update_seeker_profile(
        ...
        Args:
            ...
            update_fields: Dictionary of fields to update (can include seeker_identification)

        Returns:
            Updated Seeker object or None if update fails

        Note:
            Both seeker_id (MongoDB ObjectId) and seeker_identification (UUID) can be
            present in the Seeker model, but typically only seeker_id is used for updates.
        ...
        """
        try:
            # Fetch the seeker to get seeker_identification and current values
            ...
            # Update ChromaDB: Resume
            # Always update Resume collection (required for recommendations)
            # If resume not in update_fields, use current resume or create profile summary
            resume = update_fields.get("resume", seeker.resume)
            if not resume:
                # Create a profile summary from skills and education
                profile_parts = []
                if seeker.information.first_name and seeker.information.last_name:
                    profile_parts.append(f"{seeker.information.first_name} {seeker.information.last_name}")
                education_level = update_fields.get("education_level", seeker.education_level)
                edu_focus = update_fields.get("edu_focus", seeker.edu_focus)
                if education_level and edu_focus:
                    profile_parts.append(f"Education: {education_level} in {edu_focus}")
                elif education_level:
                    profile_parts.append(f"Education: {education_level}")
                    ...
                    profile_parts.append(f"Field of Study: {edu_focus}")
                key_skills = update_fields.get("key_skills", seeker.key_skills or [])
                if key_skills:
                    profile_parts.append(f"Skills: {', '.join(key_skills)}")
                resume = ". ".join(profile_parts) if profile_parts else "Job Seeker Profile"
            
            metadata_resume = {"Post_Date": str(updated_at)}
            update_collection(
                collection=Seeker_Resume_collection,
                collection_name="Seeker_Resume_Collection",
                id_value=str(seeker.seeker_identification),
                text_value=resume,
                metadata=metadata_resume
            )

            # Update ChromaDB: Skills
            # Get current or updated values for skills
            key_skills = update_fields.get("key_skills", seeker.key_skills or [])
            education_level = update_fields.get("education_level", seeker.education_level)
            edu_focus = update_fields.get("edu_focus", seeker.edu_focus)
